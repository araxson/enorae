...
export async function getTopServices(salonId: string, startDate: string, endDate: string, limit: number = 10) {
  ...
  // Get all appointments with services for the period
  ...
  appointments.forEach((apt: any) => {
    ...
    apt.services.forEach((svc: any) => {
      ...
      serviceStats[serviceId].revenue += svc.service.base_price || 0
    })
  })
...
export async function getTopStaff(salonId: string, startDate: string, endDate: string, limit: number = 10) {
  ...
  const staffStats: Record<string, { name: string; title: string | null; count: number; revenue: number }> = {}
  ...
  appointments.forEach((apt: any) => {
    ...
    if (!staffStats[staffId]) {
      staffStats[staffId] = {
        name: apt.staff.full_name || 'Unknown',
    ...
    staffStats[staffId].revenue += apt.total_price || 0
  })
