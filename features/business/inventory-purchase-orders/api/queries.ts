...
export type PurchaseOrderSupplier = Database['public']['Views']['suppliers']['Row']
...
export async function getPurchaseOrderReferenceData(): Promise<PurchaseOrderReferenceData> {
  await requireAnyRole(ROLE_GROUPS.BUSINESS_USERS)
  const salonId = await requireUserSalonId()
  const supabase = await createClient()
  ...
  const products = (productsResult.data || []) as PurchaseOrderProduct[]
