...
export async function updateService(
  ...
  const logger = createOperationLogger('updateService', { serviceId })
  logger.start()
  ...
  if (pricingData && hasKeys(pricingData)) {
  ...
  if (bookingRules && hasKeys(bookingRules)) {
    ...
    if (hasKeys(rulesUpdates)) {
      ...
      if (rulesError) {
        logger.error('Booking rules update failed', 'database', { error: rulesError })
        throw rulesError
      }
    }
  }
