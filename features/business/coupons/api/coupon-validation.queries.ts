...
type CouponRow = Database['catalog']['Tables']['coupons']['Row']
type CouponUsageRow = Database['catalog']['Tables']['coupon_usage']['Row']
...
export async function getCouponUsageStats(couponId: string) {
  ...
  return {
    ...
    unique_customers: new Set(data.map(u => u.customer_id)).size,
    total_discount: data.reduce((sum, u) => sum + (u.discount_amount || 0), 0),
  }
}
