...
import { getUserSalonIds } from './salon.queries'
...
export async function getDashboardMetrics(salonId: string) {
  ...
  try {
    ...
    const [appointmentsResult, staffResult, servicesResult, revenueResult] = await Promise.all([
      supabase.from('appointments').select('status, total_price').eq('salon_id', salonId),
    ...
    const appointments = (appointmentsResult.data || []) as Array<{ status: string | null; total_price: number | null }>
    ...
    const dailyMetrics = (revenueResult.data || []) as Array<{ total_revenue: number | null; metric_at: string }>
