...
// Note: .schema() is required for INSERT/UPDATE/DELETE operations since public views are read-only.
// SELECT operations can use public views (products, suppliers, etc.) without .schema().
...
export async function createProduct(salonId: string, data: z.infer<typeof productSchema>): Promise<ActionResult> {
  try {
    ...
    if (!typedSalon || typedSalon.owner_id !== session.user.id) {
