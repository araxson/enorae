...
import { validateRemoveImage } from './helpers'
...
export async function removeGalleryImage(formData: FormData): Promise<ActionResult> {
  const logger = createOperationLogger('removeGalleryImage', {})
  logger.start()
  ...
  try {
    ...
    const validation = validateRemoveImage({ salonId, imageUrl })
    ...
    if (updateError) {
      ...
      return {
        error: `Database error removing image: ${updateError.message} (Code: ${updateError.code})`,
      }
    }
    ...
    const errorMsg = error instanceof Error ? error.message : 'Unknown error removing image'
    logger.error(error instanceof Error ? error : String(error), 'system')
    return { error: `Failed to remove image: ${errorMsg}` }
  }
}
