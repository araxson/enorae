...
export async function getUserSalon(): Promise<Salon> {
  ...
  const { data, error } = await supabase
    .from('salons_view')
    .select('id, name, slug, description, owner_id, city, state_province, country, status, subscription_tier, rating_average, review_count, is_verified, is_active, created_at')
    .eq('id', salonId)
    .returns<Salon>()
    .single()
